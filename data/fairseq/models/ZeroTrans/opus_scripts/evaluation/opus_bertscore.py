...
model_id = sys.argv[2]
cuda_id = sys.argv[3]
language_sequence = ["de", "nl", "ar", "fr", "zh", "ru",]
...
for i, tgt in enumerate(language_sequence):
    ...
    for j, src in enumerate(language_sequence):
        if i == j: continue
        path_ref = os.path.join(root_path, "opus_scripts", "results", method, str(model_id), "{}-{}.r".format(src, tgt))
        path_hypo = os.path.join(root_path, "opus_scripts", "results", method, str(model_id), "{}-{}.h".format(src, tgt))
...
file = open(os.path.join(root_path, "opus_scripts", "results", method, str(model_id), "{}.bertscore".format(str(model_id))), 'w', encoding='utf-8')
