...
def import_user_module(args):
    if getattr(args, "user_dir", None) is None or args.user_dir == "":
        return
    ...
    if os.path.isfile(module_path):
        ...
        try:
            ...
            # if user dir is a sub-dir of the cwd, we try importing it as a module
            try:
                relative_path = os.path.relpath(module_path)
                if relative_path != module_path:
                    relative_module_path = ".".join(relative_path.split("/"))
                    if "." in relative_module_path:
                        relative_module_path = relative_module_path[
                            relative_module_path.find(".") + 1 :
                        ]
