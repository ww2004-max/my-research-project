    ...
    if len(checkpoints) > 0 and trainer.should_save_checkpoint_on_current_rank:
        ...
        for cp in checkpoints[1:]:
            if cfg.write_checkpoints_asynchronously:
                ...
                try:
                    if not PathManager.copy(checkpoints[0], cp, overwrite=True):
                        raise Exception(f"复制失败: {checkpoints[0]} 到 {cp}")
                except Exception as e:
                    logger.warning(f"无法复制文件 {cp}: {e}")
                    # 如果目标文件已存在，尝试使用新文件名
                    if os.path.exists(cp):
                        base, ext = os.path.splitext(cp)
                        i = 1
                        while os.path.exists(f"{base}_{i}{ext}"):
                            i += 1
                        new_cp = f"{base}_{i}{ext}"
                        logger.info(f"使用新的文件名保存检查点: {new_cp}")
                        if not PathManager.copy(checkpoints[0], new_cp, overwrite=True):
                            logger.error(f"复制到新文件名也失败: {new_cp}")
