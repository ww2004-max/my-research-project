...
def main():
    ...
    SAVE_DIR = r"C:\Users\33491\PycharmProjects\machine\pdec_work\checkpoints\europarl_5epochs"
    
    # 创建保存目录
    os.makedirs(SAVE_DIR, exist_ok=True)
    print(f"保存目录: {SAVE_DIR}")
    
    # 5个epoch训练参数 - 平衡batch size和训练时间
    sys.argv = [
        'train.py', DATA_BIN,
        '--seed', '0',
        '--fp16',
        ...
        '--warmup-updates', '2000',    # 适中的warmup
        '--max-epoch', '5',            # 5个epoch
        '--max-tokens', '2048',        # 中等batch size
        '--update-freq', '2',          # 梯度累积
    ...
    print("5个Epoch训练配置:")
    print(f"  - Epochs: 5")
    print(f"  - Max tokens per batch: 2048")
    print(f"  - Update frequency: 2 (梯度累积)")
    print(f"  - Warmup updates: 2000")
    print(f"  - Learning rate: 0.0005")
    print(f"  - 预计训练时间: 6-10小时")
    print(f"  - 每个epoch约1-2小时")
    print("=" * 60)
