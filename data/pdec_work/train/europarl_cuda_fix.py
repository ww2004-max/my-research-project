...
# -*- coding: utf-8 -*-
"""
Europarl dataset training script for PhasedDecoder (Windows路径修复版本)
...
def main():
    print("开始GPU加速的Europarl训练流程 (Windows修复版)...")
    ...
    USER_DIR = os.path.join(FAIRSEQ, "models", "PhasedDecoder")  # 绝对路径
    ...
    # 检查文件存在性
    print(f"检查USER_DIR: {USER_DIR}")
    print(f"USER_DIR存在: {os.path.exists(USER_DIR)}")
    init_file = os.path.join(USER_DIR, "__init__.py")
    print(f"__init__.py存在: {os.path.exists(init_file)}")
    ...
    train_cmd = [
        ...
        "--warmup-updates", "4000",
        "--max-epoch", "2",  # 减少epoch数进行测试
        "--max-tokens", "2000",  # 减少token数进行测试
        "--share-all-embeddings",
        "--weight-decay", "0.0001",
        "--no-epoch-checkpoints",
        "--no-progress-bar",
        "--keep-best-checkpoints", "3",
        "--log-interval", "100",  # 更频繁的日志
        "--log-format", "simple",
