...
def make_bar(sheet, start_position, metric, scores):
    ...
    sheet.cell(row=3, column=(start_position + 1)).value = round(np.sum(scores[:, 0]) / np.count_nonzero(scores[:, 0]),
                                                                 2)
    sheet.cell(row=3, column=(start_position + 2)).value = round(
        np.sum(scores[1:, 1:]) / np.count_nonzero(scores[1:, 1:]), 2)
...
def mk_table(id, method_name, work_path, metric_list, language_sequence, language_dict):
    ...
    for i in range(len(metric_list)):
        ...
        scores = extract_results_from_file(id, os.path.join(work_path, method_name), metric, language_num,
                                           language_dict)
...
# 只使用四种语言
language_sequence = ["en", "de", "es", "it"]
language_dict = {'en': 1, 'de': 2, 'es': 3, 'it': 4}
