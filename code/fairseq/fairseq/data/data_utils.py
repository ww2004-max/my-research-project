...
def batch_by_size(
    ...
    # 确保indices是numpy数组并且是int64类型
    if not isinstance(indices, np.ndarray):
        indices = np.fromiter(indices, dtype=np.int64, count=-1)
    elif indices.dtype != np.int64:
        indices = indices.astype(np.int64)

    # 确保num_tokens_vec是int64类型
    if num_tokens_vec is not None:
        if not isinstance(num_tokens_vec, np.ndarray):
            num_tokens_vec = np.fromiter(num_tokens_vec, dtype=np.int64, count=-1)
        elif num_tokens_vec.dtype != np.int64:
            num_tokens_vec = num_tokens_vec.astype(np.int64)
