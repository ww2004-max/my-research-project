...
class _FP16OptimizerMixin(object):
    ...
    def build_fp32_params(cls, args, params, flatten=True):
        ...
        if flatten:
            ...
            if is_pipeline_parallel and not use_cpu:
            ...
            for device in devices:
                ...
                if use_cpu:
                    # 在CPU模式下创建参数
                    fp32_params[device] = torch.zeros(device_param_size, dtype=torch.float32)
                else:
                    # 在GPU模式下创建参数
                    fp32_params[device] = (
...
class FP16Optimizer(_FP16OptimizerMixin, optim.FairseqOptimizer):
    ...
    def build_optimizer(cls, cfg: DictConfig, params, **kwargs):
        ...
        if use_cpu:
            ...
            fp16_optimizer_class = importlib.import_module(
                "fairseq.optim." + optimizer_name
            ).__dict__[optimizer_name.capitalize() + "Optimizer"]
            fp32_optimizer = fp16_optimizer_class.build_optimizer(cfg, params)
            return fp32_optimizer
        else:
            # 在GPU模式下，使用FP16优化器
